...
// find_central --- if true: return only one maximum, ecclude maxima at array boundaries, returns -1 if no central maximum
...
function calculateEccentricity(angles) {	
...
function calculateEccentricityfromDistances(distances) {	
	eccentricities = newArray(3);
	for (r = 0; r < 3; r++) {
		two_adjacent_points = (distances[r] + distances[r + 1] + distances[(r + 3)] + distances[(r + 4) % 6]) / 4;
		next_point = (distances[r + 2] + distances[(r + 5) % 6]) / 2;
		eccentricities[r] = two_adjacent_points / next_point;		
	}
	Array.getStatistics(eccentricities, min_eccentricity, VOID, VOID, VOID);
...
for (i = 0; i < number_particles; i++) {	
	...
	/* if six corners have been found, calculate hexagon area, eccentricity and irregularity */
	if (corners.length == 12) {
		...
		eccentricity_angles = calculateEccentricity(angles);
		setResult("Eccentricity_angles", result_counter, eccentricity_angles);
		eccentricity_distances = calculateEccentricityfromDistances(point_distances);
		setResult("Eccentricity_distances", result_counter, eccentricity_distances);
		print("Hexagon eccentricity (angles): " + eccentricity_angles + " Hexagon eccentricity (distances): " + eccentricity_distances);
